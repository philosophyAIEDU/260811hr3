-- 그로우패스 SQLite 데이터베이스 테이블 정의
-- 이 파일은 data/growthpath.db 파일을 처음 만들 때 실행되는 테이블 구조입니다.
...
CREATE TABLE IF NOT EXISTS assessments (
  assessment_id   INTEGER PRIMARY KEY AUTOINCREMENT,  -- 진단번호
  employee_id     TEXT NOT NULL,                       -- 사번
...
CREATE TABLE IF NOT EXISTS competency_scores (
  assessment_id   INTEGER NOT NULL,   -- 진단번호
...
CREATE TABLE IF NOT EXISTS recommendations (
  recommendation_id INTEGER PRIMARY KEY AUTOINCREMENT, -- 추천번호
  assessment_id      INTEGER NOT NULL,                  -- 진단번호
...
-- 설정 테이블: Gemini API 키(관리자 공용/개인 덮어쓰기)를 저장합니다.
